...
> 当前回归基线：引擎 **285 passed / 3 skipped**，WebUI **41 passed**（2026-09-16）。真实绢本工笔两张样本分类均通过；其分层质量问题已定位并修复（见下方"绢本工笔 preset 修正"）。壁布**实物样品照**新增 `textile_damask_photo` preset，8 维审计全过（旧 `textile_damask` 混用导致的三层根因已逐条处置，见下方"壁布 plate 残余项"）。通道读取缺陷已根除（统一收敛到 `engine/core/psd_layer_io.py` 单一入口，见下方"审计缺陷批次"）。
...
| 壁布（大马士革数码纹样） | `textile_damask.json` | ✅ **真·可平铺纹样实测通过（2026-09-16）**：团花 159,944px 真实检出、7 层、7/8 维过；已知 ④ 9.48%（见"壁布 plate 残余项"） |
...
> 引擎全量测试基线已更新为 **285 passed / 3 skipped**；WebUI **41 passed**。最新隔离端到端复测为 9 张样本 × cold/repeat 共 18 次，18 个 PSB 均生成、9/9 字节可复现，生产 DB 未变化；完整报告见 `docs/测试报告_20260915_全样本自适应链路收口.md`。
...
> - **已知残余（2026-09-16 两轮修复后如实记录）**：④ 内容承载 **2.817% 墨迹丢失**（初值 9.477%）。已落地两项修复：
  ① **被白名单拒绝的检出层墨迹并入「未分类残层」口径**（品类不符不成层，但内容不丢；-0.97pp）；
  ② **fabric_substrate（整幅重构底板）模式下墨迹判定与审计口径对齐（-12，原 -20）**—— 整幅重构意味着底板上每处源图墨迹都必须有层承载，-20 会使审计口径的淡墨整片丢失（-5.69pp）。
  剩余 2.82% 疑似残层「超分边缘细化」收缩所致，待像素级取证。**未放宽 ④ 阈值**。（5 个金标准 preset 均非 fabric_substrate，二者修复对既有产物字节零影响——golden_silk 前后 SHA 一致实证。）；待后续把"被拒掩模的墨迹
>   并入残层"或为壁布专设纹样检测器后复核。
> - **使用边界（结论）**：`textile_damask` = **可平铺循环数码纹样**（本样本即此）；
>   织物**实物照/特写**一律走 `textile_damask_photo`（8/8 通过）。

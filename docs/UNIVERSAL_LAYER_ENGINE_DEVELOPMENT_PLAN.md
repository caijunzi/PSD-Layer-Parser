...
> **代码基线**：2026-09-10 工作区状态（已产出 `outputs/Rosetsu_Master_16k.psb` 2.34 GB）
> **最后更新**：2026-09-10
>
> **v2.1 变更（业务方决策已确认）**：
...
> 3. **五维系统完整性与防欺骗代码自测通过**：100% 通过环境真实探活、零硬编码静态扫描、物理交付物合规、防伪代码落地与高精度时钟量化。
...
| 项 | 实测值 | 来源 |
| :--- | :--- | :--- |
| 生产交付物 | `outputs/Rosetsu_Master_16k.psb`（**2.34 GB**，`2,517,517,320` 字节） | 【实测】 |
| 几何画幅与分辨率 | **$16000 \times 7808\text{ px}$（1.25 亿像素）@ 150.0 PPI**（物理尺寸 $2709.3 \times 1322.2\text{ mm}$） | 【实测】 |
| 图层结构 | **11 个独立图层**，UI 自顶向下排序，全部具备动态紧凑最小包围盒（BBox），彻底告别全屏黑底 | 【实测】 |
...
| 合成保真度 | Section 5 预渲染图层全局平均色差 **MAE = 0.73**（远优于行业 ≤ 2.0 质检红线） | 【实测】 |
| 分层规则来源 | **全自动零硬编码**：Frangi 血管骨架流 + 自适应 Otsu 阈值 + 动态轮廓追踪（24 文件 2603 行扫描 0 违规） | 【实测】 |
...
4. **设计交付线达成"一个东西一个图层"硬约束**：全部 11 个图层具备真实的非零最小外接矩形（例如朱砂印章 BBox `[13747, 1659] -> [14048, 1940]`），彻底告别全屏黑底冗余。
...
├── outputs/                        [✅] 生产交付物（Rosetsu_Master_16k.psb 2.34 GB）
├── tests/
...
| RK-10 | **现有成品层全为画布大小、无包围盒，设计师无法拖动元素** | **高** | **✅ 已关闭（v2.4）**：11 个图层全部通过动态算子计算紧凑外接包围盒并裁剪 | M6 (已闭环) |

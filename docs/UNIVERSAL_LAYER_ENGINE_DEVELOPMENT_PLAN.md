...
> G2（Preset SSOT：`layer_semantics` 取代 layer_hierarchy，新增品类零内核改动）、
> G5（质检可回归）、G8（生成内容随产物落盘）**已达成**；ADR-007/008/011/017 已落地；
> PLATE 线产出真 CMYK（ICC 分色 + TAC 300% 合规 + 零生成内容）。**本文正文中与上述
> 状态冲突的段落以 `docs/技术尽调与代码审查报告_20260910.md` §十二（全量回归与状态对齐）
> 为准**；§7.4 性能预算对照：PLATE 276.9s / DESIGN 228.6s（CPU 神经路径）。
> **v2.1 变更（业务方决策已确认）**：

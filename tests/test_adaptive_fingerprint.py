...
class TestMaterialClassifier(unittest.TestCase):
    ...
    def test_classify_silk_painting(self):
        ...
        # 纯色模拟图没有真实工笔纹理，只验证分类器输出合法，不把它当真值。
        self.assertIn(family, ["绢本工笔", "宣纸水墨"])
    ...
    def test_classify_real_silk_truth_samples(self):
        ...
        for name in ("绢本工笔画-1.jpeg", "绢本工笔画-2.jpeg"):
            ...
            family, conf = classify_material_family(
                extract_fingerprint(image, background_mode="paper_or_gold_screen")
            )
            self.assertEqual(family, "绢本工笔", f"{name} 实际：{family} {conf:.2f}")
            self.assertGreaterEqual(conf, 0.5, f"{name} 置信度过低：{conf:.2f}")
